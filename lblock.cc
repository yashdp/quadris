...
LBlock:: LBlock(int Level){
  ...
  d = 'e';
  visited = false;
  cords[0]= new Pos(0,2,true); // This should be the first one
  cords[1]= new Pos(1,2,true);
  cords[2]= new Pos(1,1,true);
  cords[3] = new Pos(1,0,true);
  next[0]= new Pos(2,2,true); // This should be the first one
  next[1]= new Pos(2,1,true);
  next[2]= new Pos(1,1,true);
  next[3] = new Pos(0,1,true);
  prev[0]= new Pos(0,0,true); // This should be the first one
  prev[1]= new Pos(0,1,true);
  prev[2]= new Pos(1,1,true);
  prev[3] = new Pos(2,1,true);
}
//==========================  
void LBlock :: setvisited(){
  visited = true;
}
//=========================
void LBlock:: replace(int oldx, int oldy, int newx, int newy){
  for(int i = 0; i < 4; i++){
...
//=================================
void LBlock:: notifyDisplay(TextDisplay &t){
  for(int i= 0; i < 4; i++){
    t.notifymiddle(cords[i]->x, cords[i]->y, 'L');
  }
}
//========================
void LBlock:: notifyBottom(TextDisplay &t){
  for(int i = 0; i < 2; i++){
    for(int j = 0; j < 4; j++){
      t.notifybottom(i,j, ' ');
    }
  ...
  for(int i= 0; i < 4; i++){
    t.notifybottom(cords[i]->x, cords[i]->y, 'L');
  }
}
//=========================
void LBlock:: rotate(string w, TextDisplay &t){
...
//=======================
void LBlock:: move(char w, TextDisplay &t){
  for(int i=0; i < 4; i++){
    if (w == 'u'){
